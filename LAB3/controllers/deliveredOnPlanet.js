...
const locationAllService = require('./../services/deleveredOnPlanet.all')
const planetsAllService = require('./../services/planet.all')
const loadsAllService = require('./../services/load.all')
...
module.exports = {
  index (req, res) {
    res.render('pages/delivered-planet/index')
  },
  async locationList (req, res) {
    try {
      const locationList = await locationAllService()
      const planets = await planetsAllService()
      console.log(locationList);
      res.render('pages/delivered-planet/list', { locations: locationList })
    } catch (error) {
      res.render('pages/delivered-planet/list', {
        locations: [],
  ...
  async createLocationForm (req, res) {
    try {
      ...
      res.render('pages/delivered-planet/add', {
  ...
  postCreateLocation: [
    ...
    sanitizeBody('product_id').escape(),
    async (req, res) => {
      ...
      if (errors.isEmpty()) {
        try {
          ...
          res.render('pages/location/add', {
            locationTypes: locationTypes,planets: planets,
            errors: [{ msg: error.message }]
          })
        }
      } else {
